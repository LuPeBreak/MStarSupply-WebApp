...
export function useIncomeByMonth(productId: number) {
  const { transactions } = useContext(InventoryContext);

  const filterTransactionsByProductIdIncomeAndYear = transactions.filter(
    (transaction) => {
  ...
  const TotalIncomeByMonth = filterTransactionsByProductIdIncomeAndYear.reduce(
    (acc, transaction) => {
